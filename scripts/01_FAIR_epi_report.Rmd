...
header-includes: 
       ...
       \fancyhead[R]{\includegraphics[width=1.2cm]{../assets/logo.jpg}}
output:
  pdf_document:
    toc: false
    toc_depth: '2'
...
The frequent use of SQL format signalize a relatively low compliance with the FAIR data principles, with individual databases being recorded in non-standard format with a specific purpose in mind, which are not immediately interoperable with e.g. Epidoc XML based projects. `r format2$n[format2$format == "SQL or similar"]` projects use SQL as their sole data storage format, and depending on the structure of a given database, there might be a higher risk of not being easily *Findable*, *Accessible*, and *Interoperable* with the rest of the existing epigraphic datasets.
...
```{r, fig.align="center", warning=FALSE, fig.width=10, fig.cap="The chart shows the existence of institutional data policy of non-partnered projects in time."}
